-- Modified version of https://github.com/nvim-lualine/lualine.nvim/blob/master/lua/lualine/components/filename.lua
...
M.update_status = function(self)
  if self.options.ignore_filetypes[vim.bo.filetype] ~= nil then return "" end
  ...
  if data == "" then data = self.options.symbols.unnamed end
  ...
  if self.options.file_status then
    if vim.bo.modified and vim.bo.modifiable and vim.bo.readonly ~= true then
  ...
  return (
    (#symbols > 0 or data == self.options.symbols.unnamed) and table.concat(symbols, "")
    or self.options.symbols.none
  )
    .. " "
    .. data
end
